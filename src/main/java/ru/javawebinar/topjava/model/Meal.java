...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.ALL_BETWEEN,  query = "SELECT m FROM Meal m LEFT JOIN m.user u WHERE u.id=:userId AND m.dateTime >= :startDate AND m.dateTime < :endDate ORDER BY m.dateTime DESC"),
})
@Entity
